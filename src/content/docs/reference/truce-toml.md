...
| `midi2` | bool | no | Opt the plugin's MIDI ports into MIDI 2.0 / UMP. Defaults to `false` (MIDI 1.0). CLAP advertises the MIDI2 note dialect and carries the native 16/32-bit + per-note + UMP-group `EventBody` variants in and out; AU v3 declares `audioUnitMIDIProtocol` = 2.0 and emits over the host's `MIDIEventList` block (macOS 12+ / iOS 15+); VST3 maps the per-note subset through note expression; VST2 / AU v2 / AAX / LV2 stay MIDI 1.0 regardless. Requires at least one MIDI port in either direction (compile error otherwise) and applies only to directions that have one. |
| `midi2_input` | bool | no | Per-direction override of `midi2` for MIDI input. Absent follows `midi2`. `true` requires a MIDI input port (compile error otherwise). |
| `midi2_output` | bool | no | Per-direction override of `midi2` for MIDI output. Same rules as `midi2_input`, for the output direction. |
...
| Field | Default | Notes |
|-------|---------|-------|
| `formats` | plugin's default features | Formats to include when packaging. Valid: `clap`, `vst3`, `vst2`, `lv2`, `au2`, `au3`, `aax`, `standalone`. `--formats` on the CLI overrides. macOS `.pkg` and Windows Inno Setup both honor this; Linux's tarball pipeline ships every default-feature format unconditionally. |
